...
if "Error: " in latest_version:
    ...
    if not float(codeonversion) == latest_version:
        ...
        tokens = (
            ...
            'LPAREN', 'RPAREN', 'DOT', 'PRINT', 'CLEAR',
            'STRING', 'CREATEFILE', 'CREATEFOLDER', 'EDITFILE'
        )
        ...
        t_RPAREN = r'\)'
        t_DOT = r'\.'
        t_PRINT = r'print'    # Token for 'print'
        t_CLEAR = r'clear'    # Token for 'clear'
        t_CREATEFILE = r'createfile'  # Token for 'createfile'
        t_CREATEFOLDER = r'createfolder'  # Token for 'createfolder'
        t_EDITFILE = r'editfile'  # Token for 'editfile'
        t_STRING = r'\"(?:\\.|[^"\\])*\"'  # Token for string literals
        ...
        def p_statement_createfile(p):
            '''statement : CREATEFILE STRING
                        | CREATEFILE STRING STRING'''
            if len(p) == 3:
                p[0] = ('createfile', p[2], None)
            else:
                p[0] = ('createfile', p[2], p[3])

        def p_statement_createfolder(p):
            '''statement : CREATEFOLDER STRING STRING
                        | CREATEFOLDER STRING STRING STRING'''
            if len(p) == 4:
                p[0] = ('createfolder', p[2], p[3], None)
            else:
                p[0] = ('createfolder', p[2], p[3], p[4])

        def p_statement_editfile(p):
            'statement : EDITFILE STRING'
            p[0] = ('editfile', p[2])
        ...
        def p_expression_string(p):
            'expression : STRING'
            p[0] = ('string', p[1][1:-1])  # Remove the quotes
